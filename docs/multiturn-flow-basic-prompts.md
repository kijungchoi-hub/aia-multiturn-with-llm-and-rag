...
```text
[System]
각 질문에 대해 첫 검색에 사용할 문서유형 필터를 결정하라.
...
```text
[System]
질문에 직접 답하고, 핵심 근거와 필요한 예외/유의사항을 포함한 최종 답변을 생성하라.
