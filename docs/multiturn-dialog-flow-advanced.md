...
- 복합 질의 처리: 한 문장 내 다중 의도 분해 및 통합 응답
- 하이브리드 질의: `API 사실성 데이터 조회 + RAG 정책성 지식 조회` 병행
...
5. 개인화 적용 경계
- 개인화 프로필(톤/길이/형식)은 응답 합성/후처리 단계에서만 반영
- 개인화 케이스라도 질문 분해 로직은 추가 적용하지 않음

## 3) 엔티티/슬롯 표준
복합 질의 분해 시 아래 슬롯을 우선 추출한다.
...
## 5) 질문 분해 규칙 (CSV형)
개인화 케이스에서는 아래 분해 규칙을 추가로 확장하지 않고, 동일한 질의 해석 결과에 개인화 후처리만 적용한다.
...
## 9) 데모 테스트 매핑 규칙
[prompt-config.yaml](prompt-config.yaml) + [../scripts/demo-prompt-test.mjs](../scripts/demo-prompt-test.mjs)에서 CSV를 아래처럼 사용한다.
